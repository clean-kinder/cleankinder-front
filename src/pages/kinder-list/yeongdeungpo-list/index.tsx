...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/yeongdeungpo-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_yd?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_yd && data.childSchoolHygiene_yd.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_yd.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_yd &&
        data.childSchoolHygiene_yd.row && (
          <>
            <ul>
