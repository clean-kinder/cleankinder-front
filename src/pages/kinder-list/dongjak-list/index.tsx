...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/dongjak-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_dj?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_dj && data.childSchoolHygiene_dj.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_dj.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_dj &&
        data.childSchoolHygiene_dj.row && (
          <>
            <ul>
