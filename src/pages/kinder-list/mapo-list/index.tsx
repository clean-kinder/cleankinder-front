...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/mapo-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_mp?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_mp && data.childSchoolHygiene_mp.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_mp.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_mp &&
        data.childSchoolHygiene_mp.row && (
          <>
            <ul>
