...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/dobong-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_db?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_db && data.childSchoolHygiene_db.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_db.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_db &&
        data.childSchoolHygiene_db.row && (
          <>
            <ul>
