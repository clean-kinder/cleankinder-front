...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/gangdong-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_gd?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_gd && data.childSchoolHygiene_gd.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_gd.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_gd &&
        data.childSchoolHygiene_gd.row && (
          <>
            <ul>
