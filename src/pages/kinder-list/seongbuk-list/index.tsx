...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/seongbuk-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_sb?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_sb && data.childSchoolHygiene_sb.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_sb.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_sb &&
        data.childSchoolHygiene_sb.row && (
          <>
            <ul>
