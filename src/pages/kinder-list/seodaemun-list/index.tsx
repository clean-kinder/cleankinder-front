...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/seodaemun-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_sm?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_sm && data.childSchoolHygiene_sm.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_sm.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_sm &&
        data.childSchoolHygiene_sm.row && (
          <>
            <ul>
