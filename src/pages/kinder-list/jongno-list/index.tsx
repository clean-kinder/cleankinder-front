...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/jongno-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_jn?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_jn && data.childSchoolHygiene_jn.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_jn.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_jn &&
        data.childSchoolHygiene_jn.row && (
          <>
            <ul>
