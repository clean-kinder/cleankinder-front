...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/dongdaemun-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_dd?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_dd && data.childSchoolHygiene_dd.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_dd.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_dd &&
        data.childSchoolHygiene_dd.row && (
          <>
            <ul>
