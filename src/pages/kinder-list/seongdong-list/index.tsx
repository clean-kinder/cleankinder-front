...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/seongdong-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_sd?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_sd && data.childSchoolHygiene_sd.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_sd.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_sd &&
        data.childSchoolHygiene_sd.row && (
          <>
            <ul>
