...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/nowon-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_nw?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_nw && data.childSchoolHygiene_nw.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_nw.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_nw &&
        data.childSchoolHygiene_nw.row && (
          <>
            <ul>
