...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/songpa-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_sp?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_sp && data.childSchoolHygiene_sp.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_sp.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_sp &&
        data.childSchoolHygiene_sp.row && (
          <>
            <ul>
