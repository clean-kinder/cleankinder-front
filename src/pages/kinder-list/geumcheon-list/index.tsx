...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/geumcheon-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_gc?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_gc && data.childSchoolHygiene_gc.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_gc.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_gc &&
        data.childSchoolHygiene_gc.row && (
          <>
            <ul>
