...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/yangcheon-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_yc?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_yc && data.childSchoolHygiene_yc.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_yc.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_yc &&
        data.childSchoolHygiene_yc.row && (
          <>
            <ul>
