...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/gangbuk-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_gb?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_gb && data.childSchoolHygiene_gb.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_gb.row.length / itemsPerPage);
      const pageNumbers = [];
