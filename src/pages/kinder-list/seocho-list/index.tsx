...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/seocho-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_sc?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_sc && data.childSchoolHygiene_sc.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_sc.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_sc &&
        data.childSchoolHygiene_sc.row && (
          <>
            <ul>
