...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/guro-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_gr?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_gr && data.childSchoolHygiene_gr.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_gr.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_gr &&
        data.childSchoolHygiene_gr.row && (
          <>
            <ul>
