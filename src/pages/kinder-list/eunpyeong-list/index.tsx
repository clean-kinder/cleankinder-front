...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/eunpyeong-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_ep?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_ep && data.childSchoolHygiene_ep.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_ep.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_ep &&
        data.childSchoolHygiene_ep.row && (
          <>
            <ul>
