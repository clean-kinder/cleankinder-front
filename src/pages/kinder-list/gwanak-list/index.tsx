...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/gwanak-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_ga?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_ga && data.childSchoolHygiene_ga.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_ga.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_ga &&
        data.childSchoolHygiene_ga.row && (
          <>
            <ul>
