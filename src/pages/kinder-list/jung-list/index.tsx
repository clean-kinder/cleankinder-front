...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/jung-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_jg?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_jg && data.childSchoolHygiene_jg.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_jg.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_jg &&
        data.childSchoolHygiene_jg.row && (
          <>
            <ul>
