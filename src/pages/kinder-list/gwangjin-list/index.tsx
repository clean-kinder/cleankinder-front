...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/gwangjin-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_gj?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_gj && data.childSchoolHygiene_gj.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_gj.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_gj &&
        data.childSchoolHygiene_gj.row && (
          <>
            <ul>
