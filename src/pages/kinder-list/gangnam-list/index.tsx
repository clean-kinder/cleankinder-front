...
const KinderList = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/gangnam-data');
      console.log('Success!');
  ...
  const currentItems = data?.childSchoolHygiene_gn?.row?.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const renderPageNumbers = () => {
    if (data && data.childSchoolHygiene_gn && data.childSchoolHygiene_gn.row) {
      const totalPages = Math.ceil(data.childSchoolHygiene_gn.row.length / itemsPerPage);
      const pageNumbers = [];
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        data &&
        data.childSchoolHygiene_gn &&
        data.childSchoolHygiene_gn.row && (
          <>
            <ul>
